import { s as supabase } from "./index-BdFiX0VU.js";
...
async function getTodaysExecutionsByPlanIds(planIds) {
  ...
  const { data, error } = await supabase.from("training_executions").select("*").in("plan_id", planIds).gte("completed_at", startOfDay).lte("completed_at", endOfDay);
  if (error) throw error;
  return data ?? [];
}
...
export {
  getTrainingExecutions as a,
  getTrainingExecutionsByPlanId as b,
  calculateStats as c,
  deleteTrainingExecution as d,
  createTrainingExecution as e,
  getTrainingExecution as f,
  getTodaysExecutionsByPlanIds as g,
  updateTrainingExecution as u
};
//# sourceMappingURL=trainingExecutions-YCG4J-eD.js.map
